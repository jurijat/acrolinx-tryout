...
<div>
	...
	<div class="flex space-x-4 mb-4 border-b">
		<button 
			class="pb-2 px-1 transition-colors {activeTab === 'text' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}"
			onclick={() => activeTab = 'text'}
		>
			Text Input
		</button>
		<button 
			class="pb-2 px-1 transition-colors {activeTab === 'file' ? 'border-b-2 border-blue-600 text-blue-600' : 'text-gray-500 hover:text-gray-700'}"
					...
					{:else}
						<div>
							...
							<p class="text-xs text-gray-500">
								TXT, MD, HTML, XML, JSON, DOCX, PDF up to 10MB
							</p>
						</div>
					{/if}
				</label>
